...
#1. Reading files:
library(data.table)
hg38 = fread("https://ftp.ncbi.nlm.nih.gov/refseq/H_sapiens/annotation/GRCh38_latest/refseq_identifiers/GRCh38_latest_genomic.gff.gz", 
             header = F, sep = "\t", skip = 9, fill = T)
#sort(unique(hg38$V3))
orig_enh38 = hg38[hg38$V3 == "enhancer",]
#dim(orig_enh38) #81572    9
#table(orig_enh38$V2)
#Curated Genomic        RefSeqFE 
#8            81564 

##Chromosomes are identified as LOCUS (example: NC_000001). We modify this format to chr
...
#keys #"GeneID" "VISTA"  "HGNC"   "MIM"  NA  

###HGNC to HGNC crossref of HUGO
enhancers$Dbxref = gsub("HGNC:HGNC:", "https://identifiers.org/hgnc:", enhancers$Dbxref)
...
enhancers$Dbxref[indexes] = paste0("https://identifiers.org/ncbigene:", enhancers$Dbxref[indexes])

###MIM to OMIM crossref
indexes = grep("MIM:", enhancers$Dbxref)
enhancers$Dbxref = gsub("MIM:", "", enhancers$Dbxref)
enhancers$Dbxref[indexes] = paste0("http://purl.bioontology.org/ontology/OMIM/", enhancers$Dbxref[indexes])
...
dir.create("./RefSeq110_results")
save.image("./RefSeq110_results/Data_RefSeq.RData")
write.table(refseq_enh, "./RefSeq110_results/RefSeq.tsv", quote = F, sep = "\t", col.names = T, row.names = F, 
            fileEncoding = "utf8")
